...
const Portfolio = () => {
  const { portfolio, trades } = useRealTradingContext();

  // Datos de rendimiento histórico
  const performanceHistory = portfolio.performance && portfolio.performance.length > 0 ? portfolio.performance.map(data => ({ month: new Date(data.date).toLocaleDateString(), value: data.value })) : [];

  // Datos para el gráfico de pastel
  const allocationData = portfolio.assets.length > 0 ? portfolio.assets.map(asset => ({
    name: asset.symbol,
    value: asset.value,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            {performanceHistory.length > 0 ? (
              ...
              <div className="text-center text-gray-400 py-12">No hay datos disponibles para mostrar la evolución del portafolio.</div>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
            ...
            <div className="flex items-center justify-center mb-4">
              {allocationData.length > 0 ? (
                ...
                <div className="text-center text-gray-400 py-12">No hay datos disponibles para mostrar la distribución de activos.</div>
              )}
            </div>
            {allocationData.length > 0 && (
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-8">
          ...
          {portfolio.assets.length > 0 ? (
            ...
            <div className="text-center py-12 text-gray-400">
              No tienes inversiones activas. Conecta tus APIs para empezar a operar.
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
          ...
          {trades.length > 0 ? (
            <div className="space-y-4">
              {trades.map((trade) => (
                <div key={trade.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-xl">
                  <div className="flex items-center space-x-4">
                    <div className={`p-2 rounded-lg ${trade.type === 'COMPRA' ? 'bg-green-100' : 'bg-red-100'}`}>
                      {trade.type === 'COMPRA' ? 
                    ...
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${trade.type === 'COMPRA' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                          {trade.type}
                        </span>
                      </div>
            ...
            <div className="text-center py-12 text-gray-400">
              No hay operaciones registradas en tu historial. Las operaciones aparecerán aquí una vez ejecutadas.
            </div>
          )}
        </div>
